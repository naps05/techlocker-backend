...
const sendEmail = async (to, subject, html) => {
  await resend.emails.send({
    from: 'TechLocker <onboarding@resend.dev>',
    to,
    subject,
    html
  });
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td bgcolor="#1a0005" align="center" style="padding:24px 20px;border:2px solid #dc2626;">
                  <div style="font-size:10px;color:#dc2626;letter-spacing:5px;font-family:'Courier New',monospace;margin-bottom:12px;">[ ACCESS CODE ]</div>
                  <div style="font-size:52px;font-weight:900;color:#dc2626;letter-spacing:16px;font-family:'Courier New',monospace;padding-left:16px;">${code}</div>
                  <div style="margin:14px auto 0;width:200px;height:3px;background-color:#dc2626;"></div>
                  <div style="font-size:10px;color:#666666;letter-spacing:3px;font-family:'Courier New',monospace;margin-top:10px;">EXPIRES IN 10 MINUTES</div>
                </td>
              </tr>
            </table>
          </td>
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td bgcolor="#1a0008" style="padding:12px 16px;border-left:4px solid #dc2626;">
                  <div style="font-size:11px;color:#666666;font-family:'Courier New',monospace;line-height:1.7;">&#9888; ${footerNote}</div>
                </td>
              </tr>
            </table>
          </td>
...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = await User.create({
      ...
      isVerified: false, verificationCode: code, verificationExpires: expires
    });

    await sendEmail(
      email,
      '🔒 TechLocker — Initialize Your Vault',
      emailTemplate(
        '// VAULT INITIALIZATION //',
        username,
        `<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
          <tr><td style="color:rgba(255,255,255,0.45);font-size:13px;line-height:1.9;letter-spacing:0.5px;">
...
router.post('/request-email-change', authMiddleware, async (req, res) => {
  try {
    ...
    await sendEmail(
      user.email,
      '🔒 TechLocker — Email Change Request',
      emailTemplate(
        '// EMAIL CHANGE REQUEST //',
        user.username,
        `<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
          <tr><td style="color:rgba(255,255,255,0.45);font-size:13px;line-height:1.9;letter-spacing:0.5px;padding-bottom:12px;">
            A request has been made to transfer your vault access to a new email target:
          </td></tr>
          <tr><td style="background:rgba(220,38,38,0.08);border:1px solid rgba(220,38,38,0.25);border-radius:4px;padding:12px 18px;">
          ...
          <tr><td style="color:rgba(255,255,255,0.45);font-size:13px;line-height:1.9;letter-spacing:0.5px;padding-top:12px;">
...
router.post('/confirm-email-change', authMiddleware, async (req, res) => {
  try {
    const { code } = req.body;
    const user = await User.findById(req.user.id);
    if (!user.emailChangeCode) return res.status(400).json({ message: 'No email change request found' });
    if (user.emailChangeCode !== code) return res.status(400).json({ message: 'Invalid verification code' });
    if (new Date() > user.emailChangeExpires) return res.status(400).json({ message: 'Code has expired' });
    ...
    res.json({ message: 'Email updated successfully', user: { id: user._id, username: user.username, email: user.email } });
  } catch (err) {
